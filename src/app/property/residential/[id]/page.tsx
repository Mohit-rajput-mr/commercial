...
export default function ResidentialPropertyDetailPage() {
  ...
  const loadProperty = async () => {
    ...
    try {
      ...
      // If not found in database, try to load from residential datasets
      if (!details) {
        try {
          ...
          if (residentialProp) {
            const rawState = (residentialProp as any).listingState || (residentialProp as any).state || '';
            const stateLower = typeof rawState === 'string' ? rawState.toLowerCase().trim() : '';
            ...
            details = {
              ...
              images: residentialProp.images || (residentialProp.imgSrc ? [residentialProp.imgSrc] : []),
