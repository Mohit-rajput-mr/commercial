...
function JsonDetailContent() {
  ...
  // Get bit from URL path parameter and parse it
  const bitParam = typeof params.bit === 'string' ? params.bit : '';
  let bit: number | undefined;
  if (bitParam) {
    // Extract bit number from "bit123" format or use as-is if it's just a number
    if (bitParam.startsWith('bit')) {
      bit = parseInt(bitParam.replace('bit', ''));
    } else {
      bit = parseInt(bitParam);
    }
    // If parsing failed, set to undefined
    if (isNaN(bit)) {
      bit = undefined;
    }
  }
  ...
  useEffect(() => {
    const loadProperty = async () => {
      if (bit === undefined && !propertyId) {
        setError('No property ID or bit provided');
        setLoading(false);
        return;
      }
  ...
  const propertyBit = property?.bit !== undefined ? property.bit : (bit !== undefined ? bit : undefined);
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 md:gap-4 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                ...
                {propertyBit && (
                  <p className="text-gray-400 text-xs">Bit ID: {propertyBit}</p>
                )}
              </div>
            </div>
          ...
          <div className="w-full lg:w-[380px] xl:w-[420px] flex-shrink-0">
            <div className="lg:sticky lg:top-[120px]">
              <PropertyInquiryForm
                ...
                propertyId={propertyBit?.toString() || propertyId}
